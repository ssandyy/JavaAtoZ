package Thread.MovieBookingApp;

public class MovieBookingAppWithoutSynchronization extends Thread{
    static BookThreaterSeat bs;
    int seats;

    public static void main(String[] args) {
        bs=new BookThreaterSeat();
        MovieBookingAppWithoutSynchronization user1 = new MovieBookingAppWithoutSynchronization();
        user1.start();
        user1.seats=6;
        MovieBookingAppWithoutSynchronization user2 = new MovieBookingAppWithoutSynchronization();
        user2.start();
        user2.seats=5;
    }
    public void run(){
        bs.bookSeat(seats);
    }
}
class BookThreaterSeat{
    int total_seat=10;
    void bookSeat(int seats){
        if (total_seat>=seats) {
            total_seat=total_seat-seats;
            System.out.println(Thread.currentThread().getName());
            System.out.println(seats+" Seats booked Successfully..! "+" Available Seats "+total_seat);
        }else {
            System.out.println(Thread.currentThread().getName());
            System.out.println("Sorry! we are run out of seat");
            System.out.println("Available seats: "+total_seat);
        }
    }
}